Okay I think I found a sort of archetecure issue with my framework. Here is what I mean.

Right now we defined Services like so:
```ts
...
But lets imagine we have a service who behaves differently based on paramters, maybe like CORS?

well now we would need a way to essentially construct new objects (services) based on the parameters.

You could do this with some sort of object construction ordeal, but then you end up in a new issue, picture this:

```ts
services = [SomeConstructCORSService("someinput")]
async handle(c: HTTPContext) {
  // how to access service??
}
```

This creates this sort of issue where I need a way to generate dynamic services.

I guess I could do something like this:

```ts
const CORSService = SomeConstructCORSService("someinput")
services = [CORSService]
async handle(c: HTTPContext) {
  let cors = c.service(CORSService)
}
```
